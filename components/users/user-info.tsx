...
export const UserInfo: FC<UserInfoProps> = ({ user, voteStats, canEdit }) => {
  ...
  return (
    <div className="bg-neutral-200 dark:bg-neutral-900 sm:rounded-md overflow-hidden">
      <div className="grid grid-rows-[max-content_max-content] gap-2">
        <div className="w-full h-36 relative">
          {user.bannerUrl ? (
            <Image src={user.bannerUrl} alt="Edit banner" fill />
          ) : (
            <div className="w-full h-full" style={{ backgroundColor: defaultBannerColor(user.id) }}></div>
          )}
        </div>
        <div className="grid grid-cols-[max-content_max-content_auto] gap-2 p-2">
